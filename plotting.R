...
barplot(as.numeric(results_ns_s[order(as.numeric(results_ns_s[,2])),2]), col = results_ns_s[order(as.numeric(results_ns_s[,2])),8], font = 2,main = "Polymorphic sites",  space = 0,border = 0, yaxt = "n",ylim = c(0,200000))	#all
...
text(x = 41.5, y = 175000,  "Non\ncoding ", font = 1, cex = 0.8, adj = 0,srt = 45,col = "black",xpd = T)
legend(x = 0, 200000, cex = 1, legend = c("wild","weed","landrace","elite"), fill = c("dodgerblue4","gold","firebrick1","burlywood4"), bty = "o", bg = "#FFFFFF80")

####
lm_total = lm(as.numeric(results_ns_s[,2])~as.factor(results_ns_s[,1])) #classes effect on number of sites
anova(lm_total)
TukeyHSD(aov(as.numeric(results_ns_s[,2])~as.factor(results_ns_s[,1])))
...
###
par(mar = c(5,5,4,2))
barplot(as.numeric(results_ns_s[order(as.numeric(results_ns_s[,6])),6]), col = results_ns_s[order(as.numeric(results_ns_s[,6])),8], font = 2,main = expression(over(bolditalic(P)[bold(n)],bolditalic(P)[bold(s)])), ylim = c(0,0.51),space = 0,border = 0)
...
#anova(lm_ns_s)
#TukeyHSD(aov(as.numeric(results_ns_s[,6])~as.factor(results_ns_s[,1])))
beta = betareg(as.numeric(results_ns_s[,6])~as.integer(as.factor(results_ns_s[,1])),link = c("probit"));summary(beta)#beta regression
wilcox.test(as.numeric(results_ns_s[results_ns_s[,1] == "landrace",6]),as.numeric(results_ns_s[results_ns_s[,1] == "elite",6])) #landrace versus elites
wilcox.test(as.numeric(results_ns_s[results_ns_s[,1] == "landrace",6]),as.numeric(results_ns_s[results_ns_s[,1] == "wild",6])) #domesticated versus wild
wilcox.test(as.numeric(results_ns_s[results_ns_s[,1] == "elite",6]),as.numeric(results_ns_s[results_ns_s[,1] == "wild",6])) 
...
###
par(mar = c(5,5,4,2))
stop = cbind(results_ns_s[,1],as.numeric(results_ns_s[,7])/as.numeric(results_ns_s[,3]))
barplot(100*as.numeric(stop[order(as.numeric(stop[,2])),2]), col = results_ns_s[order(as.numeric(stop[,2])),8], font = 2,main = expression(over(bolditalic(P)[bold(n~(nonsense))],~bolditalic(P)[bold(n~(sense))])),space = 0,border = 0, ylim = c(0,1))
mtext(side = 2, at = max(as.numeric(stop[,2])) / 2 * 100,  text = "Proportions (X 100)", font = 1, line = 2.8, cex = 1)
...
#TukeyHSD(aov(as.numeric(stop[,2])~as.factor(stop[,1])))
#anova(lm_stop)
y = as.numeric(stop[,2]) *10
beta = betareg(y~as.integer(as.factor(stop[,1])),link = c("probit"));summary(beta)#beta regression
wilcox.test(as.numeric(stop[stop[,1] == "landrace",2]),as.numeric(stop[stop[,1] == "elite",2])) #landrace versus elites
wilcox.test(as.numeric(stop[stop[,1] == "landrace",2]),as.numeric(stop[stop[,1] == "wild",2])) #domesticated versus wild
wilcox.test(as.numeric(stop[stop[,1] == "elite",2]),as.numeric(stop[stop[,1] == "wild",2])) 
...
text(x = -0.1, y = 17.6,  "A", font = 1, cex = 1.8, adj = 0,srt = 0,col = "black",xpd = T)
...
text(x = 6, col = "red",y = 0.95,label = paste("Spearman's rho = ",signif(cor.test(sn[,2],sn[,3], method = "spearman")$estimate,2),", p-value < 2e-16",sep = ""),cex = 0.7)
dev.print(device=pdf, "results/Figure_S6_delerious_frequency_correlation.pdf", onefile=FALSE)
dev.off()
...
legend(x = 0, y = 0.3, cex = 1, legend = c("wild","landrace","elite",expression(italic(P)[n~(deleterious~bold(private))]~~"/"~~italic(P)[n~(neutral~bold(private))])), fill = c("dodgerblue4","firebrick1","burlywood4", "#99999980"), bty = "o", bg = "#FFFFFF80")


text(x = 14.7, y = 0.275, labels = expression(italic(P)[n~(deleterious)]~~"/"~~italic(P)[n~(neutral)]))
...
#snp_table_effect = snp_table_effect[1:10000,]
snp_table = read.table("results/snp_table_all3",stringsAsFactors = F, header = T)#remove the MEN landrace. It shouldn't be there...
type = colnames(snp_table)[4:ncol(snp_table)]
results_ns_s = matrix(0,nrow = length(type), ncol = 8)
colnames(results_ns_s) = c("accessions","total","non-syn","syn","nc","ns/s","STOP","color")
...
sup_table_1 = cbind(individuals[,5],type, results_ns_s[,c(2,3,4,5,7)], results_deleterious[,c(3)], results_private_deleterious[,c(2,3)])
colnames(sup_table_1)[c(1,8:10)] = c("type","ns_del.2.5","private_non.synonymous_total","private_ns_del.2.5")

sup_table_1 = sup_table_1[order(sup_table_1[,1]),]

write.table(sup_table_1,"results/sup_table_1.txt",row.names =F, col.names = T, quote =F, sep = "\t")
