...
###curating snp file ######
###########################
system(paste("wc -l results/snp_table_all2 >wordcount1", sep = ""))
wordcount = read.table("wordcount1")
mis = 0.50 #missing data threshold
con <- file(paste("results/snp_table_all2", sep = ""))

open(con) ; file = readLines(con,1); n_ind = length(strsplit(file," ")[[1]]) -3;close(con); write.table(paste(file),"results/snp_table_all3", row.names = F, col.names = F, quote = F)
...
if((missing < mis) & (ht > 0.095) & (ho < 0.6)) cat(file,file = paste("results/snp_table_all3"),append = T, fill = 1)
...
### run getorf ###
system("/Users/costanza/applications_seb/EMBOSS-6.6.0/emboss/getorf -sequence reference/HA412_trinity_noAltSplice_400bpmin_unique_ID.fa -minsize 300 -find 2 -outseq reference/HA412_trinity_noAltSplice_orf.fasta")
