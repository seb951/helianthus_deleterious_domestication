...
genes = as.numeric(args[1]) # how many genes you want to check   51468 0 == all
...
###########################
snp = function(list_ind= "reference/annuus.txt",user = "costanza",genes = 100,cpu = 4,wd = "/Volumes/backup_ubuntu_vancouver/backup_vancouver/seb/Documents/domestication_load/") {
...
if(length(grep("fq",individuals[p,1])) == 1) {individuals[p,5] = paste(individuals[p,5],".realign.sorted.bam",sep = ""); individuals[p,1] = paste(individuals[p,1],".realign.sorted.bam",sep = "")}
if(length(grep("fq",individuals[p,1])) != 1) {individuals[p,5] = paste(individuals[p,5],".fq.realign.sorted.bam",sep = ""); individuals[p,1] = paste(individuals[p,1],".fq.realign.sorted.bam",sep = "")}
} # proper format to further process with idxstats

all_final1 = paste("alignments/", individuals[1:19,5], collapse = " ", sep = "") # remove the fake individuals (the last 6) #1:21
...
all_final2 = paste("alignments/", individuals[21:41,5], collapse = " ", sep = "") # remove the fake individuals (the last 6) #22:41
...
mpileup = paste("/Volumes/backup_ubuntu_vancouver/backup_vancouver/seb/Documents/domestication_load/samtools-0.1.19/samtools mpileup -C50 -I -ugf reference/HA412_trinity_noAltSplice_400bpmin.fa ", all_final, " -r ",split[j,1]," | /Volumes/backup_ubuntu_vancouver/backup_vancouver/seb/Documents/domestication_load/samtools-0.1.19/bcftools/bcftools view -cvg - | awk 'NR > 51502' >mpileup/variants",j,".raw.vcf", sep = "")
#mpileup = paste("/Volumes/backup_ubuntu_vancouver/backup_vancouver/seb/Documents/domestication_load/samtools-0.1.19/samtools mpileup -C50 -I -ugf reference/HA412_trinity_noAltSplice_400bpmin.fa ", all_final, " -r ",split[j,1]," | /Volumes/backup_ubuntu_vancouver/backup_vancouver/seb/Documents/domestication_load/samtools-0.1.19/bcftools/bcftools view -cvg - >mpileup/variants",j,".raw.vcf", sep = "")
...
system("echo '#' >mpileup/0_51k_variants.raw.vcf") #final file
...
###Pre-cleaning step###
	...
	system("awk 'NR == 2' mpileup/0_51k_variants.raw.vcf  >mpileup/header")
...
if(genes == 0)
{
mpileup = paste("/Volumes/backup_ubuntu_vancouver/backup_vancouver/seb/Documents/domestication_load/samtools-0.1.19/samtools mpileup -C50 -I -ugf reference/HA412_trinity_noAltSplice_400bpmin.fa ", all_final," | /Volumes/backup_ubuntu_vancouver/backup_vancouver/seb/Documents/domestication_load/samtools-0.1.19/bcftools/bcftools view -cvg - | awk 'NR > 51502' >mpileup/0_51k_variants.clean.vcf", sep = "")
mpileup_exe = "mpileup/cmd/mpileup_all"
write.table(mpileup,mpileup_exe, row.names = F, col.names = F, quote = F)
system(paste("chmod +x",mpileup_exe))
system(paste("./", mpileup_exe,sep = ""))
system("wc -l mpileup/0_51k_variants.clean.vcf >mpileup/wordcount1")
wordcount = read.table("mpileup/wordcount1")
system("awk 'NR == 1' mpileup/0_51k_variants.clean.vcf  >mpileup/header")
header = read.delim("mpileup/header", header = F, stringsAsFactors = F)
header = gsub("alignments/","",header[10:length(header)], fixed = T)
header = gsub(".fq","",header, fixed = T)
write.table(t(as.matrix(c("reference", "position","reference_annuus_allele",header))),"results/snp_table_all",row.names = F, col.names = F, quote = F, sep = " ")
}
...
snp(list_ind= "reference/annuus.txt",user = "costanza",genes = genes,cpu = cpu,wd = "/Volumes/backup_ubuntu_vancouver/backup_vancouver/seb/Documents/domestication_load/")
