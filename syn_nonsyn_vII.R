...
###
if(file.exists("results/new_snp_table_effect") == F) {
system(paste("echo",names,">results/new_snp_table_effect")) #result file with column names...
for(nss in 1:nrow(snp_table))
#for(nss in 1:25000)
	{
		snp_table_effect = matrix(0,nrow = 1, ncol = 51) ### vector with info on what kind of snp it is... EMPTY IT EVERYTIME JUST TO BE SURE...
...
} else snp_table_effect = read.table("results/new_snp_table_effect", header = T, stringsAsFactors = F);snp_table_effect = snp_table_effect[,-c(23)] #remove MEN
