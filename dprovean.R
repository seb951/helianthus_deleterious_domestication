...
if(file.exists("results/dsnp_table_noX") == F) {
...
#for(i in 1:1000)
	...
	majority_temp = rle(sort(strsplit(paste(snp_table[i, (c(1:nrow(individuals))[regexpr("land|eli",individuals[,5]) > 0]+3)],collapse = ""),"")[[1]])) #replace the consensus read with a majority rule consensus nucleotide. BUT USE ONLY THE WILD INDIVIDUALS 
	...
	write.table(snp_table,"results/dsnp_table_noX", row.names = T, col.names =T,quote = F)
																	} else snp_table = read.table("results/dsnp_table_noX",stringsAsFactors = F, header = T)		
...
for(ec in 1: length(type)) 
	{
	echo_seq = paste("echo all_sequences_",type[ec]," >provean/dall_sequences_",type[ec],sep = "") #per individuals
	echo_snp = paste("echo aa_change score gene >provean/dprovean_results_",type[ec],sep = "") #per individuals
	echo_snp_all = paste("echo aa_change score gene >provean/dprovean_results_all",sep = "") #for all genes and all individuals
...
for(t in 1:length(type))
	...
	if(length(seq_all.var) > 0) {seq_all.var = unique(sort(seq_all.var));write.table(seq_all.var,paste("provean/dseq_all.var"),row.names = F, col.names = F, quote = F)}	#write all variation, if there is any
	write.table(seq.fasta,"provean/dseq.fasta",row.names = F, col.names = F, quote = F)  #write sequence a single time for all individuals.
...
#################
		...
		if(file.exists(paste("provean/dblast/set",g, sep = ""))) {system(paste("provean.sh --num_threads 7 -q provean/dseq.fasta -v provean/dseq_all.var --supporting_set provean/dblast/set",g," >provean/dprovtest.log",sep = ""))} else {system(paste("provean.sh --num_threads 7 -q provean/dseq.fasta -v provean/dseq_all.var --save_supporting_set provean/dblast/set",g," >provean/dprovtest.log", sep = ""))} #reuse the blast results if it already exists for that gene... otherwise create it for the other individuals to use it... 
		#April 30th : NOW PROVEAN RUNS WITH ONLY THE VIRIDIPLANTAE DATASET,  THE NEW NR DATABASE and version 1.1.4
			
		variants = read.csv("provean/dprovtest.log", header = F,sep = "\t", stringsAsFactors = F) #load variants
		...
		write.table(variants,"provean/dvariants",row.names = F, col.names = F, quote = F);
		system("cat provean/dvariants >>provean/dprovean_results_all");  #write sequence a single time for all individuals.system(#cat all the variants in a single file
		
		for(t in 1:length(type))
			{
				temp = variants[,1] %in% seq_ind.var[[t]]
				seq_ind.var[[t]] = variants[temp == T,] 
				write.table(seq_ind.var[[t]],"provean/dvariants",row.names = F, col.names = F, quote = F); #write it back
				if(nrow(seq_ind.var[[t]]) > 0) { 
				system(paste("cat provean/dvariants >>provean/dprovean_results_",type[t],sep = "")); # keep track of all MUTATIONS only if variants are present (seq_ind.var[[t]])
				system(paste("cat provean/dseq.fasta >>provean/dall_sequences_",type[t],sep = ""))} # keep track of all sequences only if variants are present (seq_ind.var[[t]])
			} ###result file
																											}
											}
