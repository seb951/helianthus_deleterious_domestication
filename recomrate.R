...
transcript = read.delim("~/Documents/speciation_islands_dec2011/reference/recombination_rates_all_sites", header = T, stringsAsFactors = F,sep = " ") ###recombination rates###

snp_table_effect_r = cbind(snp_table_effect,0); colnames(snp_table_effect_r)[ncol(snp_table_effect_r)] = "recomb_rate"

snp_table_effect_r = snp_table_effect_r[rowSums(snp_table_effect_r[,47:48]) != 0, ] #keep only the genes polymorphic in the domesticates
snp_table_effect_r = snp_table_effect_r[snp_table_effect_r[,44] != 0, ] #keep only the ns SNPs
...
for(r in 1:nrow(transcript))
	{
		...
		if(length(temp[temp == T]) > 0) 
			...
			temp2 = temp2[rowSums(temp2[,47:48]) > 0,] #make sure the mutation is found in the domesticates
			 transcript[r,8:9] = c(length(temp2[temp2[,45] < -2.5,1]), length(temp2[temp2[,45] != 0,1]))} # fraction of deleterious and neutrals per gene
...
for(r in 1:nrow(unique_map_transcript))
	...
	for(s in 1:100000)
		{
			ss = sample(snp_table_effect_r[,45],unique_map_transcript[r,9]) #random sample
			if((length(ss) != 0) & (unique_map_transcript[r,10] > (length(ss[ss < -2.5]) / length(ss))  ) ) resample_vector[r] = resample_vector[r] + 1
		}
	if(r %% 100000 == 0) print(paste(r,"of",nrow(unique_map_transcript),"Time is:",Sys.time()))
	resample_vector[r] = signif((100000 - resample_vector[r]) / 100000,4) # pvalue
	}
...
mtext(side = 4,expression(bolditalic(p)~bold(-value)~(~over(bolditalic(D)[bold(n~(deleterious))],~bolditalic(D)[bold(n~(neutral))]))),col = "#009900",line = 4.9,at = -3)

dev.print(device=pdf, "results/figures/Figure4_recomb_rate_LG10.pdf", onefile=FALSE)
dev.off()
