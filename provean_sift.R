...
for(ec in 1: length(type)) 
	...
	echo_seq_sift = paste("echo all_sequences_",type[ec]," >provean/new_sift_all_sequences_",type[ec],sep = "") #per individuals
	echo_snp = paste("echo aa_change score gene >provean/new_provean_results_",type[ec],sep = "") #per individuals
	echo_snp_sift = paste("echo aa_change effect score1 del_score confidence total gene >provean/new_sift_results_",type[ec],sep = "") #per individuals
	echo_snp_all_sift = paste("echo aa_change score gene >provean/new_sift_results_all",sep = "") #for all genes and all individuals
	echo_snp_all = paste("echo aa_change score gene >provean/new_provean_results_all",sep = "") #for all genes and all individuals
	
	system(echo_seq)
	system(echo_seq_sift)
	system(echo_snp)
	system(echo_snp_sift)
	system(echo_snp_all)
	system(echo_snp_all_sift)
	}
...
for(t in 1:length(type))
	{
		...
		seq.var.temp2 = c(1:nchar(seq[1]))[(strsplit(seq[2],"")$consensus == strsplit(reference_aa[2],"")$consensus) == F]
			...
			seq.var = seq.var[regexpr("X",seq.var) < 0] ### no X substitution because it does not exist...
...
#################
		...
		### ### ###SIFT### ### ### run sift instead of provean. luckily it requires the same input... (seq.fasta and a list of variants.)
		### ### ###SIFT### ### ###
		### ### ###SIFT### ### ###
		system("rm /home/seb/sift5.2.2/tmp/*")
		system("csh /home/seb/sift5.2.2/bin/SIFT_for_submitting_fasta_seq.csh provean/seq.fasta /media/seb/seb_1TB/blast_database/ncbi_protein_nr/nr  provean/seq_all.var 1>sift_templog 2>sift_templog2")
		if(file.exists("/home/seb/sift5.2.2/tmp/seq.alignedfasta") == T) {
		system("grep 'WARNING' /home/seb/sift5.2.2/tmp/seq.SIFTprediction -v >/home/seb/sift5.2.2/tmp/seq.SIFTprediction2")	 #remove the warning lines
		system("grep 'SCORE' /home/seb/sift5.2.2/tmp/seq.SIFTprediction2 -v >/home/seb/sift5.2.2/tmp/seq.SIFTprediction")	 #remove the NOT SCORE lines
		system("wc /home/seb/sift5.2.2/tmp/seq.SIFTprediction >/home/seb/sift5.2.2/tmp/wc_sift_pred") # do a wordcount check
		if(read.table("/home/seb/sift5.2.2/tmp/wc_sift_pred")[1] > 0){ # load variants only if there are some...
		variants_sift = read.csv("/home/seb/sift5.2.2/tmp/seq.SIFTprediction", header = F,sep = "\t", stringsAsFactors = F) #load variants
		variants_sift = cbind(variants_sift,paste(">",reference_aa[1],"###gene",g,sep = "")); #add the name of the sequence were the variant is from 
		write.table(variants_sift,"provean/variants_sift",row.names = F, col.names = F, quote = F);
		system("cat provean/variants_sift >>provean/new_sift_results_all");  #write sequence a single time for all individuals.system(#cat all the variants in a single file
		seq_ind.var_temp = seq_ind.var
		for(t in 1:length(type))
			{
				temp_s = variants_sift[,1] %in% seq_ind.var[[t]]
				seq_ind.var_temp[[t]] = variants_sift[temp_s == T,] 
				write.table(seq_ind.var_temp[[t]],"provean/variants_sift",row.names = F, col.names = F, quote = F); #write it back
				if(nrow(seq_ind.var_temp[[t]]) > 0) { 
				system(paste("cat provean/variants_sift >>provean/new_sift_results_",type[t],sep = "")); # keep track of all MUTATIONS only if variants are present (seq_ind.var[[t]])
				system(paste("cat provean/seq.fasta >>provean/new_sift_all_sequences_",type[t],sep = ""))} # keep track of all sequences only if variants are present (seq_ind.var[[t]])
			} ###result file
			}
			}
		### ### ###SIFT DONE HERE### ### ###
		...
		for(t in 1:length(type))
			{
				temp = variants[,1] %in% seq_ind.var_temp[[t]]
				seq_ind.var_temp[[t]] = variants[temp == T,] 
				write.table(seq_ind.var_temp[[t]],"provean/variants",row.names = F, col.names = F, quote = F); #write it back
				if(nrow(seq_ind.var_temp[[t]]) > 0) { 
